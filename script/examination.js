...
const test = (wordList, type, score, mistake, startTime, grade) => {
    ...
    wordSpace.textContent = type === 0 ? wordList[index].word : wordList[index].mean;
    ...
    do {
        ...
        choices.children[i].textContent = type === 0 ? wordList[fourIndex].mean : wordList[fourIndex].word;
    ...
    choices.addEventListener('touchend', (e) => {
        ...
        do {
            ...
            // const meanSplit = meanText(fourIndex, wordList);
            console.log(fourIndex, fourIndex < 0);
            if (fourIndex < 0) {
                fourIndex = Math.floor(Math.random() * wordList.length);
            }
...
window.addEventListener('DOMContentLoaded', () => {
    document.getElementById('startBtn').addEventListener('touchend', () => {
        ...
        const grade = [0, 250, 476, 700, 900];
        const start = selector.valaue === '6' ? 0 : grade[Number(selector.value - 1)];
        const end = selector.valaue === '6' ? 999 : grade[Number(selector.value)] - 1;
